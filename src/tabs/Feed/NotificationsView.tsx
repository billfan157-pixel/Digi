      ...
      <div className={`${glassCard} w-full h-[85vh] rounded-t-3xl flex flex-col shadow-2xl animate-in slide-in-from-bottom duration-300`}>
        <div className="p-4 border-b border-white/10 flex justify-between items-center bg-slate-900 rounded-t-3xl">
          <div className="flex items-center gap-2">
            <h3 className="text-white font-bold text-lg">{t('common.notifications')}</h3>
            {unreadCount > 0 && <span className="bg-rose-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{unreadCount} {t('common.new')}</span>}
          </div>
          <div className="flex items-center gap-2">
              {unreadCount > 0 && (
              <button onClick={markAllRead} className="text-cyan-400 text-xs font-bold hover:text-cyan-300 px-3 py-1.5 rounded-full bg-cyan-500/10 active:scale-95 transition-all">{t('feed.mark_all_read')}</button>
              )}
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {notifications.length === 0 ? (
            <div className="text-center py-10">
              ...
              <p className="text-slate-500 text-sm font-medium">{t('feed.no_notifications_yet')}</p>
            </div>
          ) : (
            <VirtualListMemo<SocialNotification>
              items={notifications}
              estimateSize={80}
              overscan={3}
              getItemKey={(n) => n.id || `notif-${n.id}`}
              ...
              renderItem={(n) => (
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center shrink-0 overflow-hidden">
                    {n.actor?.avatar_url ? <img src={n.actor.avatar_url} alt={t('feed.avatar_of_user', { name: n.actor?.nickname || t('common.user') })} className="w-full h-full object-cover" /> : (n.actor?.nickname || 'U')[0].toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-slate-200 leading-snug">
                      <span className="font-bold text-white">{n.actor?.nickname || t('feed.someone')}</span> {n.content}
                    </p>
