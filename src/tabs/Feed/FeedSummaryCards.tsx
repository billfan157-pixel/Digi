...
export const FeedSummaryCards = ({ summary }: FeedSummaryCardsProps) => {
  ...
  const primaryStats = [
    { label: '24h', value: summary.postsToday, helper: 'new', icon: TrendingUp, className: 'border-cyan-500/20 bg-cyan-500/10 text-cyan-300' },
    { label: 'Peak', value: summary.progressCount, helper: 'progress', icon: Flame, className: 'border-orange-500/20 bg-orange-500/10 text-orange-300' },
    { label: 'Duel', value: summary.challengeCount, helper: 'open', icon: Swords, className: 'border-violet-500/20 bg-violet-500/10 text-violet-300' },
  ];

  const secondaryStats = [
    { label: 'Drop', value: summary.storyCount, helper: '24h', icon: TrendingUp, className: 'border-sky-500/20 bg-sky-500/10 text-sky-300' },
  ];
  ...
  return (
    <div className="px-4 sm:px-0 space-y-2">
      {totalSignals === 0 ? (
        <div className="rounded-2xl border border-white/5 bg-slate-900/45 px-4 py-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-slate-500">{t('feed.feed_signals')}</p>
              <p className="mt-1 text-sm font-bold text-slate-300">{t('feed.no_new_activity')}</p>
            </div>
      ...
      {totalSignals > 0 && (
        ...
        >
          {isExpanded ? t('feed.hide_details') : t('feed.show_more_signals')}
          <ChevronDown
            size={14}
