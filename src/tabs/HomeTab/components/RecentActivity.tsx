...
const RecentActivity = React.memo(function RecentActivity({ waterEntries, handleDeleteEntry, setShowHistory }: RecentActivityProps) {
  ...
  if (recentEntries.length === 0) {
    return (
      <div className="text-center py-10 bg-gradient-to-br from-slate-900/40 to-slate-800/20 rounded-[1.75rem] border border-white/10 backdrop-blur-sm">
        <Droplet size={36} className="text-slate-600 mx-auto mb-3 opacity-60" />
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between px-1">
        <h3 className="text-white text-base font-black flex items-center gap-2.5">
          <History size={17} className="text-cyan-400" />
          {t('home.recent_activity')}
        </h3>
        <button 
          onClick={() => setShowHistory(true)} 
          className="text-cyan-400 text-xs font-bold hover:underline opacity-85 hover:opacity-100 transition-opacity"
        >
          {t('home.view_all')}
        </button>
      </div>
      ...
      <div className="space-y-2">
        {recentEntries.map((entry: WaterLog, index: number) => {
          ...
          return (
          <div key={entry.id || `recent-${index}`} className="group flex items-center justify-between p-3 bg-gradient-to-r from-slate-900/50 to-slate-800/30 backdrop-blur-sm border border-white/10 rounded-[1.25rem] hover:from-cyan-500/15 hover:to-cyan-600/10 hover:border-cyan-500/30 transition-all shadow-[0_2px_6px_rgba(255,255,255,0.01)] hover:shadow-[0_4px_12px_rgba(6,182,212,0.12)]">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  ...
                  {entry.name === 'DigiBottle' && (
                    <span className="text-[8px] font-bold uppercase tracking-wider text-cyan-400 bg-cyan-500/15 px-2 py-0.5 rounded-lg border border-cyan-500/30 flex items-center gap-0.5 flex-shrink-0">
                      <Bluetooth size={9} /> DigiBottle
                    </span>
                  )}
                </div>
                <p className="text-xs text-slate-400 font-medium mt-1">{formatWaterEntryTime(entry)}</p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className="text-lg font-black text-cyan-400 whitespace-nowrap">+{entry.amount}ml</span>
              <button 
                onClick={() => handleDeleteEntry(entry.id)} 
                className="w-8 h-8 rounded-lg bg-white/10 text-slate-300 hover:text-rose-400 hover:bg-rose-500/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all active:scale-90"
              >
                <X size={15} />
              </button>
            </div>
