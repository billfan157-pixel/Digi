...
const HomeTab = React.memo((props: HomeTabProps) => {
  ...
  return (
    <div ref={containerRef} className="space-y-6 animate-in fade-in zoom-in duration-300 pb-10 relative">
      <div className="absolute top-0 left-0 right-0 h-0.5 bg-slate-800/50 dark:bg-slate-900/50 z-50">
        <div className="h-full bg-cyan-500 dark:bg-cyan-400 transition-all duration-150" style={{ width: `${scrollProgress}%` }} />
      </div>
      <HomeHeader 
        profile={profile} 
          ...
          <div className="absolute text-center z-10 drop-shadow-lg pointer-events-none flex flex-col items-center">
            ...
            <div className="mt-2 px-3 py-1.5 bg-white/60 dark:bg-slate-900/60 backdrop-blur-md rounded-full border border-slate-200 dark:border-white/10 flex items-center gap-1.5 shadow-sm">
              <span className="text-[10px] text-slate-600 dark:text-slate-300 font-bold uppercase tracking-widest">
                {t('home.goal')}: <span className="text-slate-800 dark:text-white font-black">{waterGoal} ml</span>
              </span>
            </div>
            {progress >= 100 && (
            ...
            <div>
              ...
              {effectiveIsConnected ? (
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-emerald-500 dark:text-emerald-400 text-[10px] font-black uppercase tracking-widest flex items-center gap-1">
                    <BatteryFull size={10} /> {batteryLevel}%
                  </span>
          ...
          <div key="drink-menu-modal" className="fixed inset-0 z-[100] flex flex-col justify-end">
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6">
                ...
                <button onClick={() => { setIsDrinkMenuOpen(false); setIsCustomMode(false); }} className="p-2 bg-white/5 rounded-full text-slate-400 hover:text-white active:scale-95 transition-all">
                  <X size={18} />
                </button>
              </div>
                ...
                {isCustomMode ? (
                  <motion.div key="custom-view" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.2 }} className="flex flex-col gap-4 mb-2">
                    <input 
                      ...
                      className="w-full bg-slate-800/50 backdrop-blur-md border border-white/5 rounded-2xl p-4 text-white font-bold outline-none focus:border-cyan-500" 
                    />
                    <div className="bg-slate-800/50 backdrop-blur-md border border-white/5 rounded-2xl p-4">
                      <div className="flex justify-between items-center mb-3">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('volume')}</label>
                        <span className="text-2xl font-black text-cyan-400">{customVolume}ml</span>
                      </div>
