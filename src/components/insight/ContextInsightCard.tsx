...
const COLOR_MAP: Record<ContextInsight['category'], { bg: string; border: string; text: string; iconBg: string }> = {
  ...
  day_pattern: { bg: 'bg-cyan-500/10', border: 'border-cyan-500/20', text: 'text-cyan-400', iconBg: 'bg-cyan-500/15' },
...
export default function ContextInsightCard({ insight, index }: ContextInsightCardProps) {
  ...
  return (
    <motion.div
      ...
      className={`rounded-[var(--theme-border-radius-inner,12px)] border ${colors.border} ${colors.bg} backdrop-blur-sm p-4`}
    >
      <div className="flex items-start gap-3">
        <div className={`w-9 h-9 rounded-[var(--theme-border-radius-inner,8px)] ${colors.iconBg} flex items-center justify-center shrink-0`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mt-2.5">
            ...
            <span className={`text-[10px] font-bold ${isNegative ? 'text-orange-400' : 'text-emerald-400'}`}>
