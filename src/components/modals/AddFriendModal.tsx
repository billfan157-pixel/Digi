    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-6" style={{ background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(4px)' }} onClick={() => { setShowAddFriend(false); setSearchQuery(''); setSearchResults([]); }}>
      <div className="w-full max-w-sm rounded-3xl p-6" style={{ background: '#1e293b', border: '1px solid rgba(16,185,129,0.3)' }} onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-5">
          <div><h3 className="text-xl font-black text-white">{t('add_friend.title')}</h3></div>
        ...
        <div className="relative mb-6">
          <input type="text" value={searchQuery} onChange={e => handleSearchUser(e.target.value)} placeholder={t('common.enter_nickname')} className="w-full p-3.5 pl-10 rounded-xl bg-slate-900 border border-slate-700 text-white text-sm outline-none focus:border-emerald-500" />
          ...
          <div className="space-y-3 mb-6">
            {searchResults.map((user, index) => (
              <div key={user.id || `add-friend-res-${index}`} className="flex items-center justify-between p-3 rounded-xl bg-slate-800 border border-slate-700">
                <div className="flex items-center gap-3"><div className="w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-inner" style={{ background: 'linear-gradient(135deg, #10b981, #059669)' }}>{(user.nickname || 'U').charAt(0).toUpperCase()}</div><div><p className="text-white text-sm font-bold">{user.nickname || 'User'}</p><p className="text-slate-400 text-[10px]">DigiWell User</p></div></div>
                <button onClick={() => handleAddFriend(user.id, user.nickname)} className="px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-xs font-bold active:scale-95 transition-all">Add Friend</button>
              </div>
            ))}
          </div>
        ) : (<div className="text-center py-6"><Users size={32} className="text-slate-600 mx-auto mb-3" /><p className="text-slate-400 text-sm">{searchQuery.length > 2 ? 'User not found' : 'Search by nickname to compete together'}</p></div>)}
      </div>
    </div>
  );
}