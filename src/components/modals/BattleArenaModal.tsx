...
const RELEASE_BATTLE_ACTIONS_ENABLED = false;
...
export default function BattleArenaModal() {
  ...
  const loadArenaData = useCallback(async () => {
    ...
    try {
      ...
      // 2. Tải danh sách người dùng để thách đấu (Ghép trận thông minh ngang tầm)
      if (!currentActive) {
        const { data: users } = await supabase
          .from('public_profiles')
          .select('id, nickname, level, avatar_url')
          .neq('id', profile.id)
          .gte('level', Math.max(1, (profile.level || 1) - 5))
          .lte('level', (profile.level || 1) + 5)
          .limit(10);
          
        if (!users || users.length === 0) {
          const { data: fallback } = await supabase.from('public_profiles').select('id, nickname, level, avatar_url').neq('id', profile.id).limit(10);
          setOpponents(fallback || []);
        } else {
          setOpponents(users);
        }
      }
  ...
  // --- THE KILLER FEATURE: LIVE SYNC (CẬP NHẬT THỜI GIAN THỰC KHI ĐỐI THỦ UỐNG NƯỚC) ---
  useEffect(() => {
    if (!activeBattle || !profile?.id) return;
    
    const opponentId = activeBattle.challenger_id === profile.id ? activeBattle.opponent_id : activeBattle.challenger_id;

    const channel = supabase
      .channel('live_battle_sync')
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'public_profiles', filter: `id=eq.${opponentId}` },
        (payload: any) => {
          const newWater = payload.new?.water_today;
          if (newWater !== undefined) {
            setActiveBattle(prev => {
              if (!prev) return prev;
              const isOpponentChallenger = prev.challenger_id === opponentId;
              const oldWater = isOpponentChallenger ? prev.challenger?.water_today : prev.opponent?.water_today;
              
              if (newWater > (oldWater || 0)) {
                if (navigator.vibrate) navigator.vibrate([50, 50, 50]);
                toast.warning(`CẢNH BÁO: Đối thủ vừa nạp thêm nước! Tiến độ mới: ${newWater}ml`, { icon: '⚠️' });
              }
              
              if (isOpponentChallenger) return { ...prev, challenger: { ...prev.challenger, water_today: newWater } };
              return { ...prev, opponent: { ...prev.opponent, water_today: newWater } };
            });
          }
        }
      )
  ...
  }, [activeBattle?.id, profile?.id]);

  // --- ĐỒNG BỘ LƯỢNG NƯỚC CỦA CHÍNH MÌNH VÀO TRẬN ĐẤU KHI UỐNG ---
  useEffect(() => {
    if (profile && activeBattle) {
      setActiveBattle(prev => {
        if (!prev) return prev;
        const isChallenger = prev.challenger_id === profile.id;
        const currentWater = isChallenger ? prev.challenger?.water_today : prev.opponent?.water_today;
        if (currentWater !== profile.water_today) {
          if (isChallenger) return { ...prev, challenger: { ...prev.challenger, water_today: profile.water_today } };
          return { ...prev, opponent: { ...prev.opponent, water_today: profile.water_today } };
        }
        return prev;
      });
    }
  }, [profile?.water_today, activeBattle?.id]);

  // --- GỬI LỜI MỜI THÁCH ĐẤU ---
  const handleChallenge = async (opponentId: string) => {
    if (!RELEASE_BATTLE_ACTIONS_ENABLED) {
      toast.error('Đấu trường tạm khóa trong build public cho tới khi stake được xử lý atomically ở backend.');
      return;
    }
    if (!profile) return;
  ...
  const handleAccept = async (battle: Battle) => {
    if (!RELEASE_BATTLE_ACTIONS_ENABLED) {
      toast.error('Nhận kèo tạm khóa trong build public để tránh lệch xu giữa client và server.');
      return;
    }
    if (!profile) return;
      ...
      >
        ...
        {isLoading ? (
          ...
          <div className="relative z-10">
            {!RELEASE_BATTLE_ACTIONS_ENABLED && !activeBattle && (
              <div className="mb-4 rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-sm text-amber-200">
                Tạo và nhận kèo đang tạm khóa trong build public này. Trận đấu chỉ được mở lại khi stake coin được backend xác nhận atomically.
              </div>
            )}
                      ...
                      {pendingInvites.map(invite => (
                        <div key={invite.id} className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 flex justify-between items-center">
                          ...
                          <button
                            ...
                            disabled={!RELEASE_BATTLE_ACTIONS_ENABLED}
                            className="px-4 py-2 bg-amber-500 text-slate-950 font-black text-xs rounded-lg active:scale-95 transition-transform shadow-[0_0_15px_rgba(245,158,11,0.3)] disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            Nhận kèo
                          </button>
                        </div>
                      ...
                      opponents.map(user => (
                        <div key={user.id} className="bg-slate-800/40 border border-slate-700/50 hover:border-rose-500/30 transition-colors rounded-xl p-3 flex justify-between items-center">
                          ...
                          <button 
                            ...
                            disabled={!RELEASE_BATTLE_ACTIONS_ENABLED}
                            className="w-8 h-8 flex items-center justify-center rounded-full bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white active:scale-95 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
                          >
                            <Swords size={14} />
                          </button>
                        </div>
