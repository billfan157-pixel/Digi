...
const BottomNav = (props: BottomNavProps) => {
  ...
  if (bottleDemoEnabled) {
    navItems.splice(3, 0, { id: 'bottle', icon: FlaskConical, label: 'My Bottle' });
  }
  ...
  return (
    <nav aria-label="Điều hướng chính" className="absolute bottom-0 left-0 right-0 px-4 pt-10 bg-gradient-to-t from-slate-950 via-slate-950/80 to-transparent pointer-events-none pb-[calc(env(safe-area-inset-bottom,1.5rem)+0.5rem)]">
      <div role="tablist" className="flex items-center justify-between bg-slate-900/80 backdrop-blur-2xl border border-white/10 rounded-[2rem] px-2 py-2 shadow-[0_20px_50px_rgba(0,0,0,0.5)] mx-auto max-w-md pointer-events-auto relative">
