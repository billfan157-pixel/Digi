...
export const getFrameEffects = (level: number) => {
  ...
  // Mặc định: Khởi Động (< 15) - Chỉ border mờ
  let config = {
    title: "KHỞI ĐỘNG",
  ...
  // Cấp 15-29: Duy Trì - Border sáng nhẹ
  if (safeLevel >= 15) {
    config = {
      title: "DUY TRÌ",
  ...
  // Cấp 30-49: Tối Ưu - Border xanh đậm, shadow tỏa nhiệt
  if (safeLevel >= 30) {
    config = {
      title: "TỐI ƯU",
  ...
  // Cấp 70-99: Tinh Hoa (Hào Quang Vàng)
  if (safeLevel >= 70) {
    config = {
      title: "TINH HOA",
  ...
  // Cấp 100+: Hoàn Mỹ (Hiệu ứng Dòng chảy Cyan)
  if (safeLevel >= 100) {
    config = {
      title: "HOÀN MỸ",
