...
export function useBehaviorAnalysis({ weeklyData, waterLogs, waterGoal }: UseBehaviorAnalysisProps) {
  ...
  const patterns = useMemo((): BehaviorPattern[] => {
    ...
    const morningAvg = weeklyData
      .filter((_, i) => i < 4) // Mon-Thu
      .reduce((s, d) => s + d.ml, 0) / Math.min(4, weeklyData.length);
    const eveningAvg = weeklyData
      .filter((_, i) => i >= 4) // Fri-Sun
      .reduce((s, d) => s + d.ml, 0) / Math.max(1, weeklyData.length - 4);
    ...
    const completedDays = weeklyData.filter(d => d.ml >= waterGoal).length;
    const consistency = completedDays / weeklyData.length;
    ...
    const weekdays = weeklyData.slice(0, 5);
    const weekends = weeklyData.slice(5);
