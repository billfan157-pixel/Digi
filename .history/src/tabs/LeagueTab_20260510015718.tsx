...
import { Trophy, UserPlus, Zap, Crown, Users, Target, Search } from 'lucide-react';
...
const LeagueTab = memo(function LeagueTab({
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 relative min-h-screen pb-6 bg-slate-950">
      {/* Background glow */}
      <div className="fixed top-0 left-0 w-full h-80 bg-gradient-to-b from-cyan-600/8 via-blue-500/4 to-transparent pointer-events-none blur-2xl" />
      ...
      <div className="px-6 mt-4 space-y-4">
        ...
        {leagueMode === 'clubs' ? (
          currentUser?.id ? <ClubsView userId={currentUser.id} /> : <EmptyState searchQuery={searchQuery} leagueMode={leagueMode} onReset={() => { setSearchQuery(''); setLeagueView('all'); }} onAddFriend={setShowAddFriend} />
        ) : (
