...
import { Trophy, UserPlus, Zap, Crown, Flame, Search, Radar, Sparkles, Users, Target, Swords } from 'lucide-react';
...
const LeagueTab = memo(function LeagueTab({
  ...
  const EmptyState = useCallback(() => (
    <div className="rounded-3xl border border-white/5 bg-slate-900/60 p-8 text-center backdrop-blur-xl">
      ...
      <div className="mt-5 flex items-center justify-center gap-3">
        <button
          onClick={() => { setSearchQuery(''); setLeagueView('all'); }}
  ...
  ), [searchQuery, leagueMode, setShowAddFriend]);
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 relative min-h-screen pb-6 bg-slate-950">
      ...
      <div className="px-6 mt-4 space-y-4">
        ...
        {leagueMode === 'clubs' ? (
          currentUser?.id ? <ClubsView userId={currentUser.id} /> : <EmptyState />
        ) : (
