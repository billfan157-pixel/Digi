      ...
      {isOpen && (
        <div key="main-menu-sidebar" className="fixed inset-0 z-[100] flex justify-end">
          ...
          <motion.div 
            ...
            className="relative w-64 h-full bg-slate-900/90 backdrop-blur-xl border-l border-white/5 shadow-2xl flex flex-col"
          >
            <div className="p-6 flex items-center justify-between border-b border-white/5">
              <h2 className="text-white font-black text-lg">{t('home.menu_title')}</h2>
              <button onClick={onClose} className="p-2 bg-white/5 rounded-full text-slate-400 hover:hover:text-white active:scale-95 transition-all">
                <X size={18} />
              </button>
            </div>
            <div className="p-4 space-y-6 flex-1 overflow-y-auto scrollbar-hide">
              ...
              {profile && (
                <div 
                  ...
                  className="flex items-center gap-3 bg-slate-800/50 p-3 rounded-2xl border border-white/5 cursor-pointer hover:hover:bg-slate-800/80 active:scale-95 transition-all shadow-sm"
                >
                  <AvatarFrame 
                    size="sm" 
                  ...
                  />
                  <div>
                    <h3 className="text-white font-bold text-base leading-tight">{profile.nickname || t('home.you')}</h3>
                    <p className="text-cyan-400 text-[10px] font-bold uppercase tracking-wider mt-0.5">Cấp độ {profile.level || 1}</p>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-emerald-500/10 border border-emerald-500/20 p-3 rounded-2xl flex flex-col items-center justify-center gap-1 shadow-sm">
                    <ShieldCheck size={20} className="text-emerald-400" />
                    <span className="text-emerald-400 font-black text-lg">
                      <CountUp value={profile?.wp || 0} />
                    </span>
                    <span className="text-[9px] text-emerald-500/70 font-bold uppercase tracking-widest text-center">Điểm sức khỏe</span>
                  </div>
                  <div className="bg-amber-500/10 border border-amber-500/20 p-3 rounded-2xl flex flex-col items-center justify-center gap-1 shadow-sm">
