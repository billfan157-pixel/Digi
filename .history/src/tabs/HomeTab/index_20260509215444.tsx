...
import { getRandomRewardMessage, getStreakTier } from '../../utils/healthMath';
...
const HomeTab = React.memo((props: HomeTabProps) => {
  ...
  // Variable rewards: show random toast when user logs water
  const previousIntakeRef = useRef(waterIntake);
  useEffect(() => {
    if (waterIntake > previousIntakeRef.current && waterIntake > 0) {
      const streak = profile?.total_exp ? Math.max(Math.floor(profile.level || 0), 1) : 0;
      const msg = getRandomRewardMessage(streak);
      toast.success(msg, { id: 'reward-message', duration: 2000 });
    }
    previousIntakeRef.current = waterIntake;
  }, [waterIntake, profile]);
  ...
  return (
    <div ref={containerRef} className="space-y-5 animate-in fade-in zoom-in duration-300 pb-12 relative">
      ...
      <div className="px-5 -mb-2 mt-2">
        <StreakTierBadge streak={profile ? Math.max(profile.level || 0, 0) : 0} size="sm" showNext />
      </div>
