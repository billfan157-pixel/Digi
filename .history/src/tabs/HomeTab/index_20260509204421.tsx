  ...
  return (
    <div ref={containerRef} className="space-y-5 animate-in fade-in zoom-in duration-300 pb-12 relative">
      ...
      {!effectiveIsConnected ? (
        <>
          {/* Tap ring to log 250ml quickly | Hold for drink menu */}
          <div
            className="relative my-8 flex flex-col items-center justify-center cursor-pointer active:scale-95 transition-transform px-6"
            onClick={() => {
              handleAddWater(250, 1, 'Nuoc Loc');
              toast.success('+250ml', { duration: 1500 });
            }}
            onDoubleClick={() => {
              handleAddWater(500, 1, 'Nuoc Loc');
              toast.success('+500ml', { duration: 1500 });
            }}
            onContextMenu={(e) => {
              e.preventDefault();
              setIsDrinkMenuOpen(true);
            }}
          >
            ...
            <div className="absolute text-center z-10 drop-shadow-xl pointer-events-none flex flex-col items-center">
              ...
              {/* Quick tap hint - fades after 3 seconds */}
              <motion.span
                initial={{ opacity: 0.6 }}
                animate={{ opacity: 0 }}
                transition={{ delay: 4, duration: 1 }}
                className="mt-3 text-[9px] text-cyan-400/70 font-bold uppercase tracking-widest"
              >
                Chạm để uống 250ml
              </motion.span>
              {progress >= 100 && (
              ...
              {/* Tap goal badge to see detail */}
              <div
                onClick={(e) => {
                  e.stopPropagation();
                  setShowGoalDetail(true);
                }}
                className="mt-3 cursor-pointer active:scale-90 transition-transform"
              >
                <span className="text-[9px] text-cyan-400/50 font-bold uppercase tracking-widest hover:text-cyan-400/80 transition-colors">
                  Chi tiết
                </span>
              </div>
            </div>
