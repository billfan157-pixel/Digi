...
const HomeTab = React.memo((props: HomeTabProps) => {
  ...
  // UX Audit Fix: Hide Bottle Demo card in production to avoid looking like an engineering prototype
  const bottleDemoEnabled = import.meta.env.DEV;
      ...
      {!effectiveIsConnected ? (
        <div className="relative my-8 flex flex-col items-center justify-center cursor-pointer active:scale-95 transition-transform px-6" onClick={() => setShowGoalDetail(true)}>
          ...
          <div className="absolute text-center z-10 drop-shadow-xl pointer-events-none flex flex-col items-center">
            ...
            <button 
              onClick={(e) => { e.stopPropagation(); setShowGoalDetail(true); }}
              className="mt-4 px-4 py-1.5 bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/10 rounded-full text-xs font-bold text-white pointer-events-auto shadow-sm transition-colors"
            >
              Chi tiết mục tiêu
            </button>
            {progress >= 100 && (
