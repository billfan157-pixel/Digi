import { Swords, Trophy, Zap, Target, Clock, TrendingUp, X, ChevronRight, Coins, Shield, Flame, Activity, Loader2, Crown } from 'lucide-react';
...
const ArenaTab = memo(({ profile }: ArenaTabProps) => {
    ...
    const fetchArenaData = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .select(`*, challenger:public_profiles!hydration_battles_challenger_public_profile_fkey(id, nickname, avatar_url, water_today, water_goal), opponent:public_profiles!hydration_battles_opponent_public_profile_fkey(id, nickname, avatar_url, water_today, water_goal)`)
  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 pb-6 pt-2">
      {/* Header */}
      <TabHeader
        label="KHU VỰC THÁCH ĐẤU"
        title={<span className="flex items-center gap-3">Võ Đài <Swords size={24} className="text-red-400 drop-shadow-[0_0_12px_rgba(239,68,68,0.5)]" /></span>}
        profile={profile}
...
const BattleCard = ({ battle, profile, now, onClick }: BattleCardProps) => {
...
const BattleDetailModal = ({ battle, profile, now, onClose }: BattleDetailModalProps) => {
  const isChallenger = battle.challenger_id === profile?.id;
  const me = isChallenger ? battle.challenger : battle.opponent;
  const opponent = isChallenger ? battle.opponent : battle.challenger;
  const userNickname = me?.nickname ?? 'Bạn';
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="flex justify-between items-center pt-2 pb-3 px-6 relative z-20">
        <div className="flex-1 pr-4">
          <p className="section-title text-slate-400 mb-1.5">KHU VỰC THÁCH ĐẤU</p>
          <h1 className="text-3xl font-black tracking-tighter text-white leading-tight flex items-center gap-3">
            Võ Đài <Swords size={24} className="text-red-400 drop-shadow-[0_0_12px_rgba(239,68,68,0.5)]" />
          </h1>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button className="w-11 h-11 rounded-xl bg-slate-800/60 backdrop-blur-md border border-white/10 flex items-center justify-center text-rose-400 hover:bg-rose-500/15 active:scale-90 transition-all duration-200 ease-out shadow-sm hover:shadow-[0_0_15px_rgba(244,63,94,0.2)] group">
            <TrendingUp size={18} className="group-hover:scale-110 transition-transform" />
          </button>
          <button className="rounded-full active:scale-90 transition-all duration-200 ease-out flex items-center justify-center hover:scale-105">
        ...
        <div className="grid grid-cols-2 gap-3 mb-6 relative z-10">
          ...
          <div className="rounded-2xl border border-slate-700/50 bg-slate-800/30 p-3 flex flex-col items-center justify-center">
            ...
            <p className="text-lg font-black text-white">
              {Math.floor((Math.max(0, endsAt.getTime() - now)) / (1000 * 60 * 60))}h
            </p>
          </div>
        </div>
