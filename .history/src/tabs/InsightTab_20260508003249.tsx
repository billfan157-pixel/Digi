...
import HydrationContextCard from '../components/HydrationContextCard';
...
const InsightTab = memo(function InsightTab({
  ...
  const { profile, isPremium, waterGoal, weeklyHistory: weeklyChartData, streak, hydrationResult, waterIntake, waterEntries, isWatchConnected, isWeatherSynced, isCalendarSynced } = useAppStore(useShallow((state: any) => ({
    ...
    <div className="space-y-6 pb-28 animate-in fade-in slide-in-from-bottom-5 duration-700">
      ...
      <div className="pt-8 pb-2 px-6">
        <div className="flex justify-between items-center mb-5">
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/20 w-fit">
            <Activity size={12} className="text-cyan-400" />
            <span className="text-[9px] font-black uppercase tracking-widest text-cyan-400">Health Intelligence</span>
          </div>
          <button onClick={() => {}} className="active:scale-95 transition-transform">
      ...
      {/* --- LAYER 3.5: ANALYTICS LAYER (PHASE 1) --- */}
      <div className="px-6 mb-8">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-black text-white uppercase tracking-wider flex items-center gap-2">
            <Activity size={16} className="text-indigo-400" /> Advanced Analytics
          </h3>
        </div>
      ...
      <div className="px-6 mt-8">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="col-span-2 bg-slate-900/60 backdrop-blur-xl border border-white/5 rounded-2xl p-4 flex items-center justify-between">
             <div>
