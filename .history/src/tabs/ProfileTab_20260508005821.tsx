  ...
  return <div className="animate-in slide-in-from-right duration-300 space-y-5 pb-6">
    <div className="flex justify-between items-start pt-6 pb-4 px-6">
      ...
      <div className="flex items-center gap-3">
        {!isPremium ? <button onClick={() => setShowPremiumModal(true)} className="bg-purple-600 text-white rounded-full px-4 py-2 text-xs font-bold active:scale-95 transition-all duration-200 ease-out hover:bg-purple-700">
            Upgrade PRO
          </button> : <div className="px-3 py-1.5 rounded-lg text-[10px] font-bold text-amber-400 bg-amber-500/10 border border-amber-500/30 flex items-center gap-1">
    ...
    <div className="flex border-b border-white/10 mb-4 px-2">
      <button onClick={() => setActiveView('stats')} className={`flex-1 pb-3 text-sm font-bold flex items-center justify-center gap-2 transition-all relative ${activeView === 'stats' ? 'text-cyan-500 dark:text-cyan-400' : 'text-slate-500 hover:text-slate-400'}`}>
      ...
      <button onClick={() => setActiveView('posts')} className={`flex-1 pb-3 text-sm font-bold flex items-center justify-center gap-2 transition-all relative ${activeView === 'posts' ? 'text-slate-800 dark:text-white' : 'text-slate-500 hover:text-slate-400'}`}>
