...
import { Share2, Plus, RefreshCw, Heart, Zap, Target, MoreHorizontal, MessageCircle, Globe, Droplets, Swords, Award, Filter, CheckCircle2, Loader2, X, Send, Trash2, Flag, Edit2, Flame, Bell, CloudSun, HeartPulse, Coffee, Bookmark, Trophy, Search, ArrowUpRight, Radar, Sparkles } from 'lucide-react';
...
const stableRange = (seed: string, min: number, max: number) => {
  const span = max - min + 1;
  return min + (stableHash(seed) % span);
};

const storySeed = (story: SocialFeedPost, suffix: string) =>
  `${suffix}:${story.id || story.author_id || story.created_at || story.content || 'story'}`;

const getFallbackStoryPercent = (story: SocialFeedPost) => {
...
const getFallbackStoryTemperature = (story: SocialFeedPost) =>
  [32, 28, 35][stableHash(storySeed(story, 'story-temp')) % 3];

const getFallbackStoryDrink = (story: SocialFeedPost) => {
...
const postSeed = (postId: string | undefined, index: number, suffix: string) =>
  `${suffix}:${postId || `post-${index}`}`;

const sortPostsByLatest = (posts: SocialFeedPost[]) =>
...
const FeedTab = memo(function FeedTab({
  ...
  const [feedMode, setFeedMode] = useState<'smart' | 'latest' | 'following'>('smart');
  ...
  const finalRankedFeed = useMemo(() => {
    ...
    // @ts-ignore
    let ranked = feedMode === 'latest'
      ? sortPostsByLatest(normalizedPosts)
      : rankFeedPosts(normalizedPosts, socialFollowingIds, profile);
    ...
    <div className="animate-in slide-in-from-right duration-300 pb-8 relative bg-slate-950 h-full overflow-y-auto scrollbar-hide">
      ...
      <div className="sticky top-0 z-30 flex flex-col pt-6 pb-4 px-6 bg-slate-950/90 backdrop-blur-xl border-b border-white/5">
        <div className="flex justify-between items-start mb-4">
            <div>
              <p className="text-[10px] font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase mb-1">TRẠM PHÁT TIN</p>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setShowDiscoverPeople(true)} className="hidden sm:flex items-center gap-1.5 px-3 py-2 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-300 text-xs font-bold hover:bg-cyan-500/20 transition-colors active:scale-95">
          ...
          <button onClick={() => setShowSocialProfile(true)} className="rounded-full flex items-center justify-center active:scale-95 transition-all shadow-lg shadow-black/20 dark:shadow-white/10 hover:shadow-cyan-500/20">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide pb-1">
            <button onClick={() => setFeedMode('smart')} className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all ${feedMode === 'smart' ? 'bg-cyan-500 text-white shadow-[0_0_10px_rgba(6,182,212,0.35)]' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10'}`}>
                Smart
            </button>
            <button onClick={() => setFeedMode('latest')} className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all ${feedMode === 'latest' ? 'bg-white text-slate-900' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10'}`}>
                Mới nhất
            </button>
            <button onClick={() => setFeedMode('following')} className={`px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap transition-all ${feedMode === 'following' ? 'bg-emerald-500 text-white shadow-[0_0_10px_rgba(16,185,129,0.35)]' : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-white/10'}`}>
                Đang follow
            </button>
        ...
        <div className="mt-3 flex items-center gap-2 rounded-2xl bg-slate-900/70 border border-white/5 px-3 py-2">
          ...
          />
          {feedSearch && (
            <button onClick={() => setFeedSearch('')} className="text-slate-500 hover:text-white transition-colors">
              <X size={16} />
            </button>
          )}
        </div>
      ...
      <div className="max-w-[600px] mx-auto mt-4 space-y-5 pb-12">
        <div className="grid grid-cols-3 gap-3 px-4 sm:px-0">
          <div className="rounded-2xl border border-cyan-500/20 bg-cyan-500/10 p-3">
            ...
            <p className="mt-2 text-2xl font-black text-white">{feedSummary.postsToday}</p>
          ...
          <div className="rounded-2xl border border-orange-500/20 bg-orange-500/10 p-3">
            ...
            <p className="mt-2 text-2xl font-black text-white">{feedSummary.progressCount}</p>
          ...
          <div className="rounded-2xl border border-purple-500/20 bg-purple-500/10 p-3">
            ...
            <p className="mt-2 text-2xl font-black text-white">{feedSummary.challengeCount}</p>
        ...
        <div className="pt-1 pb-3 border-b border-white/5">
            <div className="px-4 mb-3 flex items-center justify-between">
              <div>
                <p className="text-sm font-black text-white">Hydration stories</p>
                <p className="text-xs text-slate-500">{feedSummary.storyCount} vòng story đang hoạt động</p>
              </div>
        ...
        {!socialError && !isLoading && finalRankedFeed.length === 0 && (
          <div className="bg-slate-900/50 border border-white/5 rounded-3xl shadow-lg p-8 text-center backdrop-blur-sm mt-8">
            ...
            <div className="flex items-center justify-center gap-3">
              <button onClick={() => { setFeedFilter('all'); setFeedMode('smart'); }} className="py-2.5 px-6 rounded-xl bg-white/5 border border-white/10 text-white text-sm font-bold active:scale-95 transition-all hover:bg-white/10">
                  Reset feed
              </button>
              <button onClick={() => setShowDiscoverPeople(true)} className="py-2.5 px-6 rounded-xl bg-cyan-500/15 border border-cyan-500/20 text-cyan-300 text-sm font-bold active:scale-95 transition-all hover:bg-cyan-500/25">
                  Khám phá người mới
              </button>
            </div>
          </div>
        ...
        {/* Render Feed */}
        {isLoading && finalRankedFeed.length === 0 && (
