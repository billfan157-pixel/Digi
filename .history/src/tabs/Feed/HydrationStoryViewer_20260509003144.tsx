    ...
    >
      ...
      <div className="absolute bottom-12 left-6 right-6 z-30 flex flex-col gap-4 pointer-events-none">
        ...
        <div className="flex flex-wrap gap-2">
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }} className="flex items-center gap-1.5 px-3 py-2 bg-black/40 backdrop-blur-xl rounded-2xl border border-white/20 shadow-xl"><Droplets size={16} className="text-cyan-400" /><span className="text-white font-bold text-sm">Đạt {pct}% mục tiêu</span></motion.div>
          <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }} className="flex items-center gap-1.5 px-3 py-2 bg-black/40 backdrop-blur-xl rounded-2xl border border-white/20 shadow-xl"><CloudSun size={16} className="text-amber-400" /><span className="text-white font-bold text-sm">Thời tiết {temp}°C</span></motion.div>
          ...
          {(story.streak_snapshot || 0) > 0 && <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.4 }} className="flex items-center gap-1.5 px-3 py-2 bg-black/40 backdrop-blur-xl rounded-2xl border border-white/20 shadow-xl"><Zap size={16} className="text-orange-400" /><span className="text-white font-bold text-sm">Chuỗi {story.streak_snapshot} ngày</span></motion.div>}
        </div>
      </div>
    </motion.div>
  );
