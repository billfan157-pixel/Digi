...
export const PostCard = memo(({ post, currentUserId, handleToggleLikePost, onOpenComments }: PostCardProps) => {
  ...
  const handleSaveClick = async () => {
    ...
    const success = await toggleSavePost(post.id);
    if (!success) {
      // Already reverted in hook
    }
  };

  const handleCheersClick = async () => {
    const success = await cheersPost(post);
  ...
  const handleJoinChallengeClick = async () => {
    await joinChallenge(post.author_id);
  };
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 relative z-20">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 shadow-inner bg-slate-800 border-2 border-slate-700/50" style={{ background: isChallenge ? 'linear-gradient(135deg, #a855f7, #6366f1)' : 'linear-gradient(135deg, rgba(16,185,212,0.35), rgba(6,182,212,0.25))' }}>
              ...
              {showMenu && (
                <div key="post-menu-overlay">
                  ...
                  >
                    {isMyPost ? (
                      <>
                        <button onClick={handleEditClick} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-slate-300 hover:bg-white/5 transition-colors"><Edit2 size={16} /> Chỉnh sửa</button>
                        <button onClick={handleDeleteClick} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-rose-400 hover:bg-rose-500/10 transition-colors"><Trash2 size={16} /> Xóa bài viết</button>
                      </>
                    ) : (
                      <button onClick={handleReportClick} className="w-full flex items-center gap-3 px-4 py-3 text-sm text-amber-400 hover:bg-amber-500/10 transition-colors"><Flag size={16} /> Báo cáo vi phạm</button>
                    )}
                  </motion.div>
                </div>
              )}
