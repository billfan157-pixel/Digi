...
const InsightTab = memo(function InsightTab({
  ...
  const handleSecretClick = useCallback(() => {
    ...
    clickTimeoutRef.current = setTimeout(() => setSecretCount(0), 2000);

    setSecretCount(prev => {
      const next = prev + 1;
      if (next === 3) toast.info('Nhấp 2 lần nữa để mở khóa PRO', { id: 'secret-toast' });
      if (next === 4) toast.info('Nhấp 1 lần nữa để mở khóa PRO', { id: 'secret-toast' });
      
      if (next >= 5) {
        const currentStatus = useAppStore.getState().isPremium;
        useAppStore.setState({ isPremium: !currentStatus });
        toast.success(currentStatus ? 'Đã TẮT chế độ PRO' : '🚀 Đã MỞ KHÓA chế độ PRO thành công!');
    ...
    <div className="space-y-6 pb-28 animate-in fade-in duration-300">
      ...
      <div className="flex justify-between items-start pt-6 pb-4 px-6">
        ...
        <div onClick={handleSecretClick} className="cursor-pointer select-none touch-manipulation">
          ...
          <h1 onClick={handleSecretClick} className="text-3xl font-black tracking-tight text-slate-900 dark:text-white select-none active:scale-95 transition-transform origin-left">
