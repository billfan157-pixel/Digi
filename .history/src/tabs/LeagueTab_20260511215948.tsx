  ...
  return (
    <div className="animate-in slide-in-from-right duration-300 relative min-h-screen pb-6 bg-slate-950">
      ...
      <div className="sticky top-0 z-30 bg-slate-950/80 backdrop-blur-xl border-b border-white/[0.04]">
        ...
        {/* Stats mini bar - nâng cấp */}
        <div className="grid grid-cols-3 gap-2.5 px-6 pb-4">
          {[
            { label: 'Đối thủ', value: sortedData.length, accent: 'cyan', icon: Users },
            { label: 'Streak nóng', value: hottestStreak, accent: 'orange', icon: Zap },
            { label: 'WP TB', value: averageWp.toLocaleString(), accent: 'emerald', icon: TrendingUp },
          ].map((stat) => (
            <div
              key={stat.label}
              className={`relative overflow-hidden rounded-xl border border-${stat.accent}-500/15 bg-${stat.accent}-500/[0.06] p-3 backdrop-blur-xl group`}
            >
              <div className={`absolute -top-3 -right-3 w-12 h-12 bg-${stat.accent}-400/10 blur-[25px] rounded-full`} />
              <div className="flex items-center justify-between mb-1">
                <p className={`text-[9px] font-black uppercase tracking-widest text-${stat.accent}-300/80`}>
                  {stat.label}
                </p>
                <stat.icon size={12} className={`text-${stat.accent}-300/50`} />
              </div>
