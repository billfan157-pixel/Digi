...
const InsightTab = memo(function InsightTab({
  ...
  useEffect(() => {
    ...
    if (saved) {
      try { setCustomSchedule(JSON.parse(saved)); } catch(e) {}
    } else if (hydrationResult?.schedule && customSchedule.length === 0) {
      setCustomSchedule(hydrationResult.schedule);
    }
  }, [hydrationResult, profile?.id]);
  ...
  const handleSaveSchedule = async () => {
    ...
    try {
      ...
      if (perm.display !== 'granted') {
        toast.warning('Sếp chưa cấp quyền thông báo nên app không thể rung được nhé!');
        return;
      }
      ...
      const notificationsToSchedule = customSchedule.map((item, index) => {
        ...
        return {
          id: 1000 + index, // Gắn ID định danh
          title: '💧 DigiWell - Đến giờ nạp nước!',
          body: item.note || `Đã đến mốc ${item.time}. Hãy nạp ${item.amount}ml nước sếp nhé!`,
          schedule: { on: { hour, minute }, allowWhileIdle: true }, // allowWhileIdle: Cho phép rung kể cả khi máy đang khóa màn hình
          sound: equippedSound, // 🎵 Cấu hình âm thanh tùy chỉnh
          actionTypeId: 'SCHEDULE_REMINDER_ACTIONS', // Liên kết thông báo với bộ nút bấm vừa tạo
      ...
      toast.success("✅ Đã kích hoạt Push Notification cho lịch trình!");
    } catch (err) {
  ...
  return (
    <div className="space-y-6 pb-28 animate-in fade-in slide-in-from-bottom-5 duration-700">
              ...
              <div className="space-y-3 pb-4">
                ...
                {isEditingSchedule && (
                  <button onClick={handleAddScheduleItem} className="w-full py-3.5 mt-2 rounded-xl border border-dashed border-cyan-500/40 text-cyan-400 bg-cyan-500/10 text-sm font-bold flex items-center justify-center gap-2 hover:bg-cyan-500/20 transition-all active:scale-95">
                    <PlusCircle size={18} /> Thêm mốc giờ mới
                  </button>
                )}
              </div>
            </motion.div>
      ...
      <AnimatePresence>
        {isScheduleOpen && (
          <div key="schedule-manager-modal" className="fixed inset-0 z-[200] flex items-end justify-center bg-slate-950/80 backdrop-blur-sm" onClick={() => setIsScheduleOpen(false)}>
            <motion.div
              initial={{ y: '100%' }}
              animate={{ y: 0 }}
              exit={{ y: '100%' }}
            ...
            >
              ...
              <div className="flex items-center justify-between mb-6 shrink-0">
                ...
                <button onClick={() => setIsScheduleOpen(false)} className="p-2 rounded-full bg-slate-800 text-slate-400 hover:text-white transition-colors">
                  <X size={20} />
                </button>
              </div>
              ...
              <div className="flex justify-end mb-4 shrink-0">
                {!isEditingSchedule ? (
                  ...
                  <button onClick={() => {
                      setIsEditingSchedule(false);
                      toast.success("Đã lưu cấu hình lịch trình mới!");
                    }} 
                    className="px-4 py-2 rounded-lg bg-emerald-500 text-slate-950 text-xs font-bold shadow-[0_0_15px_rgba(16,185,129,0.4)] active:scale-95 transition-all flex items-center gap-1">
                    <Save size={14} /> Lưu lại
                  </button>
              ...
              <div className="space-y-3 overflow-y-auto pr-2 scrollbar-hide flex-1 pb-10">
                {customSchedule.length > 0 ? (
                  customSchedule.map((item, index) => (
                    <div key={index} className={`p-4 rounded-2xl border backdrop-blur-md transition-all flex items-start sm:items-center justify-between ${isEditingSchedule ? 'bg-slate-800/80 border-cyan-500/40 shadow-lg' : 'bg-slate-800/40 border-white/5'}`}>
                      <div className="flex items-start sm:items-center gap-3 flex-1 min-w-0">
