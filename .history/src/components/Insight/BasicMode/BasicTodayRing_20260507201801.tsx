...
import { Droplets, CheckCircle2, TrendingUp, TrendingDown, Sparkles } from 'lucide-react';
...
function ConfettiParticle({ delay }: { delay: number }) {
  // Dùng useMemo để khóa giá trị random, tránh hạt bị nhảy vị trí khi re-render
...
export default function BasicTodayRingUpgraded({
  ...
  const comparisonPercent = yesterdayIntake > 0 
    ? Math.round((comparison / yesterdayIntake) * 100) 
    : 0;
  ...
  useEffect(() => {
    if (isCompleted && !prevCompletedRef.current) {
      if (navigator.vibrate) {
        navigator.vibrate([50, 50, 100]); // Haptic feedback khi 100%
      }
    ...
    <div className="px-6">
      <style>{`
        ...
        @keyframes glow-pulse {
          0%, 100% { filter: drop-shadow(0 0 8px var(--neon-cyan, rgba(56,189,248,0.4))); }
          50% { filter: drop-shadow(0 0 16px var(--neon-cyan, rgba(56,189,248,0.6))); }
        }
        ...
        .ring-glow {
          filter: drop-shadow(0 0 ${glowIntensity * 12}px var(--neon-cyan, #38bdf8));
        }
        
        .gradient-text {
          background: linear-gradient(135deg, var(--neon-cyan, #38bdf8) 0%, var(--neon-cyan, #818cf8) 100%);
        ...
        .number-glow {
          text-shadow: 0 0 ${glowIntensity * 20}px var(--neon-cyan, #38bdf8);
        }
      `}</style>
      ...
      <div className="bg-slate-900/55 backdrop-blur-xl border border-white/10 rounded-3xl shadow-xl p-6 relative overflow-hidden">
        ...
        {/* Ring visualization */}
        <div 
          className="flex justify-center pt-0 pb-2 -mt-4 cursor-pointer"
          role="button"
          tabIndex={0}
            ...
            >
              <defs>
                <linearGradient id="weeklyGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="var(--neon-cyan, #2dd4bf)" />
                  <stop offset="100%" stopColor="var(--neon-cyan, #0d9488)" />
                </linearGradient>
                <linearGradient id="dailyGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="var(--neon-cyan, #38bdf8)" />
                  <stop offset="100%" stopColor="var(--neon-cyan, #818cf8)" />
                </linearGradient>
                <filter id="glow">
              ...
              {[25, 50, 75, 100].map((milestone) => {
                ...
                return (
                  <circle
                    ...
                    cy={y}
                    r="2"
                    fill={dailyPercent >= milestone ? "var(--neon-cyan, #38bdf8)" : "rgba(148,163,184,0.2)"}
