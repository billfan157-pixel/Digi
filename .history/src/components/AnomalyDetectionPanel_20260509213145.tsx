  ...
  const anomalyAnalysis = useMemo(() => {
    ...
    if (values.length >= 2 && secondLastDay && lastDay < secondLastDay * 0.6) {
      anomalies.push({
        ...
        color: 'text-red-400'
        severity: 'medium',
        message: `Hôm nay cơ thể đang cần thêm nước đó!`,
        suggestion: `Chỉ cần thêm vài ly nữa là lấy lại phong độ như hôm qua.`,
        icon: Zap,
    ...
    if (belowGoalDays >= 2) {
      anomalies.push({
        ...
        color: 'text-amber-400'
        severity: 'medium',
        message: `Đang có chút chững lại trong ${belowGoalDays} ngày qua`,
        suggestion: 'Đừng lo! Hãy thiết lập nhắc nhở để dễ dàng đạt mục tiêu hơn nhé.',
        icon: Target,
        color: 'text-cyan-400'
      });
    }
