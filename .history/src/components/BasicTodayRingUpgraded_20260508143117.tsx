  ...
  return (
    <div className="px-6">
      ...
      <div className="bg-slate-900/55 backdrop-blur-xl border border-white/10 rounded-3xl shadow-xl p-5 pb-4 relative overflow-hidden">
          ...
          <div className="relative w-64 h-64">
            ...
            >
              ...
              {[25, 50, 75, 100].map((milestone) => {
                ...
                const x = 128 + innerRadius * Math.cos(angle - Math.PI / 2);
                const y = 128 + innerRadius * Math.sin(angle - Math.PI / 2);
                return (
                  <circle
                    key={milestone}
                    cx={x}
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-4">
              ...
              <p className="text-6xl font-black text-white number-glow gradient-text">
              <p className="text-5xl leading-none tracking-tight font-black text-white number-glow gradient-text mb-0.5">
                {animatedPercent}%
              </p>
        ...
        <div className="grid gap-3 sm:grid-cols-2 mt-1 relative z-10">
          {/* Current goal card */}
        ...
        {/* Combined Stats Pill */}
        <div className="flex items-stretch justify-center mt-2 relative z-10 bg-slate-900/40 backdrop-blur-xl border border-white/5 rounded-2xl shadow-lg transition-all hover:bg-slate-900/50 hover:border-white/10">
          {/* Current goal segment */}
          <div className="flex-1 p-2.5 flex flex-col justify-center items-center text-center">
            <p className="text-[8px] uppercase tracking-[0.2em] text-slate-400 font-bold mb-0.5">
              Mục tiêu hiện tại
            </p>
            ...
            <p className="text-base font-black text-white">
              {isCompleted ? (
                ...
                <>
                  ...
                  <span className="text-[9px] text-slate-500 font-medium ml-0.5">ml</span>
                </>
              )}
            </p>
                ...
                {comparison > 0 ? (
                  <>
                    ...
                    <span className="text-[8px] text-emerald-400">
                      +{Math.abs(comparison).toLocaleString('vi-VN')} ml
                    </span>
                  </>
                ) : (
                  <>
                    ...
                    <span className="text-[8px] text-orange-400">
                      {comparison.toLocaleString('vi-VN')} ml
                    </span>
                  </>
                )}
              </div>
          ...
          <div className="flex-1 p-2.5 flex flex-col justify-center items-center text-center">
            ...
            <p className="text-base font-black text-white">
              ...
              <span className="text-[9px] text-slate-500 font-medium ml-0.5">%</span>
            </p>
            
            {/* Mini sparkline trend */}
            {weeklyTrend.length > 0 && (
              <div className="flex items-center gap-0.5 mt-1">
              <div className="flex items-center gap-0.5 mt-1 opacity-80">
                {weeklyTrend.slice(-7).map((value, i) => {
                  const height = Math.max((value / 100) * 16, 2);
                  const height = Math.max((value / 100) * 12, 2);
                  return (
                    <div
                      key={i}
