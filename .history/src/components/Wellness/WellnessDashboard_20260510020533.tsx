...
function useConfettiStyle(index: number) {
  const style = useMemo(() => {
    ...
    const r2 = (Math.sin(seed * 1.3 + 7.5) * 43758.5453) % 1;
    const r3 = (Math.sin(seed * 1.7 + 3.1) * 43758.5453) % 1;
    const r4 = (Math.sin(seed * 2.1 + 11.9) * 43758.5453) % 1;
    const x = (r1 - 0.5) * 260;
    const y = r2 * 320 + 180;
    const rotation = r3 * 720 - 360;
    const scale = r4 * 0.5 + 0.45;
