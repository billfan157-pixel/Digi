...
- Routing structure
- Database schema (migrations only via SQL, not ORM)
- Existing TypeScript interfaces — extend, don't replace

**File conventions:**
- Hooks → src/hooks/use[Name].ts
- Components → src/components/[Feature]/[Name].tsx  
- Supabase queries → src/lib/[feature].ts (not inline in components)
- No business logic inside JSX — extract to hooks

**Supabase rules:**
- Always handle both `data` and `error` from every query
- Never expose service_role key — only anon key in frontend
...
- Button variants: primary | ghost | danger | icon-btn only
